...
namespace ProductionListener.ExternalServices
{
    public class ExternalOrderService : BaseExternalService, IExternalOrderService
    {
        ...
        public async Task UpdateProcessJobStatus(Guid orderId, UpdateProcessJobStatusRequest request)
        {
            var uri = $"{_baseServiceUrl}/{API_UPDATE_PROCESS_JOB_STATUS(orderId)}";
